...
use crate::types::{FrameCount, Order, Pfn};
...
impl PageInfo {
    ...
    fn mark_free(&mut self, order: Order) {
        self.state = PageState::Free;
        self.order = order
    }
    fn mark_allocated(&mut self, order: Order) {
...
pub struct BuddyAllocator {
    ...
    dram_base: PhysAddr,
    nr_frames: FrameCount,
    free_area: [Option<Pfn>; MAX_ORDER],
    free_frames: FrameCount,
}
...
impl BuddyAllocator {
    ...
    pub fn alloc_pages(&mut self, order: Order) -> Option<PhysAddr> {
        self.alloc(order).map(|pfn| self.pa_of(pfn))
    }

    pub fn alloc_page(&mut self) -> Option<PhysAddr> {
        self.alloc_pages(Order::new(0))
    }

    pub fn free_pages(&mut self, pa: PhysAddr, order: Order) {
    ...
    fn push_front(&mut self, order: Order, pfn: Pfn) {
        debug_assert!(self.page(pfn).is_free());
        let old_head = self.free_area[order.index()];
        ...
        self.free_area[order.index()] = Some(pfn);
    }

    fn unlink(&mut self, order: Order, pfn: Pfn) {
        ...
        match prev {
            ...
            None => self.free_area[order.index()] = next,
        }
    ...
    fn pop_front(&mut self, order: Order) -> Option<Pfn> {
        let head = self.free_area[order.index()]?;
        self.unlink(order, head);
        Some(head)
    }

    fn alloc(&mut self, order: Order) -> Option<Pfn> {
        let mut k = (order.index()..MAX_ORDER).find(|&k| self.free_area[k].is_some())?;
        let block = self.pop_front(Order::new(k as u8)).unwrap();
        while k > order.index() {
            k -= 1;
            let buddy = Pfn::new(block.raw() + (1u32 << k));
            self.page_mut(buddy).mark_free(Order::new(k as u8));
            self.push_front(Order::new(k as u8), buddy);
        }
        self.free_frames -= 1usize << order.raw();
        self.page_mut(block).mark_allocated(order);
        Some(block)
    }

    fn free(&mut self, pfn: Pfn, order: Order) {
        self.free_frames += 1usize << order.raw();

        let mut cur = pfn;
        let mut cur_order = order;

        while cur_order.index() < MAX_ORDER - 1 {
    ...
    pub fn free_range(&mut self, start: Pfn, count: FrameCount) {
        ...
        while pfn < end {
            ...
            let mut i = 1u32;
            while i < (1u32 << order.raw()) {
                self.page_mut(Pfn::new(pfn + i)).mark_tail();
                i += 1;
            }
            let head = Pfn::new(pfn);
...
mod verification {
    ...
    fn math_only(dram_base: usize, nr_frames: usize) -> BuddyAllocator {
        BuddyAllocator {
            ...
            nr_frames: FrameCount::new(nr_frames),
            free_area: [None; MAX_ORDER],
            free_frames: FrameCount::new(0),
        }
    }
    ...
    fn buddy_is_involution() {
        ...
        let idx: u32 = kani::any();
        kani::assume((idx as usize) < nr_frames);
        let order = Order::new(kani::any());
        kani::assume(order.index() < MAX_ORDER);
    ...
    fn push_front_links_head() {
        ...
        assert!(a.free_area[order.index()] == Some(p2));
    ...
    fn push_pop_round_trip() {
        ...
        let o = Order::new(kani::any());
        kani::assume(o.index() < MAX_ORDER);
        let i: u32 = kani::any();
        kani::assume(i < 4);
        let p = Pfn::new(i);
        ...
        assert!(a.free_area[o.index()] == None);
    ...
    fn unlink_head() {
        ...
        let (p0, p1) = (Pfn::new(i0), Pfn::new(i1));

        a.push_front(o, p0);
        a.push_front(o, p1); // head: p1 -> p0
        a.unlink(o, p1);

        assert!(a.free_area[o.index()] == Some(p0));
    ...
    fn unlink_tail() {
        ...
        assert!(a.free_area[o.index()] == Some(p1));
    ...
    fn unlink_middle() {
        ...
        // list is now p2 -> p0
        assert!(a.free_area[o.index()] == Some(p2));
    ...
    fn alloc_splits_correctly() {
        ...
        a.page_mut(Pfn::new(3)).mark_tail();
        a.page_mut(f0).mark_free(Order::new(2));
        a.push_front(Order::new(2), f0);
        a.free_frames = FrameCount::new(4);

        let o: u8 = kani::any();
        kani::assume((o as usize) <= 2);

        assert!(a.alloc(Order::new(o)) == Some(f0));
        assert!(a.page(f0).state() == PageState::Allocated);
        assert!(a.page(f0).order() == Order::new(o));
        ...
        match o {
            ...
            1 => {
                ...
                assert!(a.page(f2).order() == Order::new(1));
            }
            _ => {
                assert!(a.free_area[1] == Some(f2));
                assert!(a.free_area[0] == Some(f1));
                assert!(a.page(f2).order() == Order::new(1));
                assert!(a.page(f1).order() == Order::new(0));
            }
        }

        assert!(a.free_frames == FrameCount::new(4 - (1usize << o)));
    }
    ...
    fn free_no_coalesce() {
        ...
        a.page_mut(f0).mark_allocated(Order::new(0));
        a.free_frames = FrameCount::new(0);

        a.free(f0, Order::new(0));

        assert!(a.free_area[0] == Some(f0));
        assert!(a.page(f0).state() == PageState::Free);
        assert!(a.page(f0).order() == Order::new(0));
        assert!(a.free_frames == FrameCount::new(1));
    }
    ...
    fn free_upper_buddy_coalesces() {
        ...
        a.page_mut(Pfn::new(2)).mark_reserved();
        a.page_mut(Pfn::new(3)).mark_reserved();

        a.page_mut(f0).mark_free(Order::new(0));
        a.push_front(Order::new(0), f0); // free_area[0] = 0
        a.page_mut(f1).mark_allocated(Order::new(0));
        a.free_frames = FrameCount::new(1);

        a.free(f1, Order::new(0));

        assert!(a.free_area[1] == Some(f0)); // head is the lower PFN, not f1
        assert!(a.free_area[0] == None);
        assert!(a.page(f0).state() == PageState::Free);
        assert!(a.page(f0).order() == Order::new(1));
        assert!(a.page(f1).state() == PageState::Tail);
        assert!(a.free_frames == FrameCount::new(2));
    }
    ...
    fn alloc_free_round_trip() {
        ...
        a.page_mut(Pfn::new(1)).mark_tail();
        a.page_mut(Pfn::new(2)).mark_tail();
        a.page_mut(Pfn::new(3)).mark_tail();
        a.page_mut(f0).mark_free(Order::new(2));
        a.push_front(Order::new(2), f0);
        a.free_frames = FrameCount::new(4);
        ...
        let block = a.alloc(Order::new(o)).unwrap();
        a.free(block, Order::new(o));
        ...
        assert!(a.free_area[0] == None);
        assert!(a.page(f0).state() == PageState::Free);
        assert!(a.page(f0).order() == Order::new(2));
        assert!(a.free_frames == FrameCount::new(4));
    }
    ...
    fn reserve_all(a: &mut BuddyAllocator) {
        let mut i = 0u32;
        while i < 4 {
            a.page_mut(Pfn::new(i)).mark_reserved();
            i += 1;
        }
    }
    ...
    fn free_range_full_is_one_block() {
        ...
        a.free_range(Pfn::new(0), FrameCount::new(4));

        assert!(a.free_area[2] == Some(Pfn::new(0)));
        assert!(a.free_area[1] == None);
        assert!(a.free_area[0] == None);
        assert!(a.page(Pfn::new(0)).state() == PageState::Free);
        assert!(a.page(Pfn::new(0)).order() == Order::new(2));
        assert!(a.page(Pfn::new(1)).state() == PageState::Tail);
        assert!(a.page(Pfn::new(2)).state() == PageState::Tail);
        assert!(a.page(Pfn::new(3)).state() == PageState::Tail);
        assert!(a.free_frames == FrameCount::new(4));
    }
    ...
    fn free_range_unaligned_tail() {
        ...
        a.free_range(Pfn::new(1), FrameCount::new(3));

        assert!(a.free_area[0] == Some(Pfn::new(1)));
        assert!(a.free_area[1] == Some(Pfn::new(2)));
        assert!(a.free_area[2] == None);
        assert!(a.page(Pfn::new(1)).order() == Order::new(0));
        assert!(a.page(Pfn::new(2)).order() == Order::new(1));
        assert!(a.page(Pfn::new(3)).state() == PageState::Tail);
        assert!(a.free_frames == FrameCount::new(3));
    }
    ...
    fn free_range_conserves_mass() {
        ...
        a.free_range(Pfn::new(start), FrameCount::new(count));

        assert!(a.free_frames == FrameCount::new(count));
    }
    ...
    fn new_starts_reserved() {
        ...
        let mut a = BuddyAllocator::new(pages, PhysAddr::new(0x4000_0000), FrameCount::new(4));

        let mut i = 0u32;
        while i < 4 {
            assert!(a.page(Pfn::new(i)).state() == PageState::Reserved);
            i += 1;
        }
        assert!(a.free_frames == FrameCount::new(0));
        assert!(a.alloc(Order::new(0)) == None);
    }
